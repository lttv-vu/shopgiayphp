<div class="row content-cart">
									...
									<?php foreach ($cart as $key => $value) :
										...
										<tr>
											...
											<td>
												...
												<input name="quantitycheck" id="check<?php echo $row['id'] ?>" class="form-control" type="hidden" value="<?php echo ($row['number'] - $row['number_buy'] ) ?>">
													<input name="quantity" style="width : 80px !important" id="<?php echo $row['id'] ?>" class="form-control" type="number" value="<?php echo $value ?>"  required onchange="onChangeSL(<?php echo $row['id'] ?>)">
												</div>
											</td>
											<td>
												<span class="amount">
	...
	<script>
		function onChangeSL(id){
			...
			var slcheck = "check" + stringid;
			var sl = document.getElementById(id).value;
			var check = document.getElementById(slcheck).value;
			if(parseInt(sl) <= 0){
				alert("Số lượng không hợp lệ!");
				location.reload()
			}
			else if(parseInt(sl) > parseInt(check)){
